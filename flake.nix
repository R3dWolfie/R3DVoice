      ...
      perSystem =
        ...
        in
        {
          packages.R3DVoice = pkgs.stdenv.mkDerivation (finalAttrs: {
            ...
            buildPhase = ''
              runHook preBuild
              pnpm --filter @r3dvoice/shared build
              pnpm --filter @r3dvoice/server exec prisma generate
              pnpm --filter @r3dvoice/server build
              pnpm --filter @r3dvoice/client run build
              runHook postBuild
            '';
